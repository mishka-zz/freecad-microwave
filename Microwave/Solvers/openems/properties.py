...
def _kind(obj: Any) -> str:
    ...
    This is a copy of ``Objects.kinds.kind_of``, and the only copy. It cannot
    import that function, because ``Objects/__init__.py`` imports FreeCAD and
...
def stale_document(error: AttributeError, objects: Iterable[Any]) -> TranslationError | None:
    ...
    restored object against the class it belongs to, so a document written by an
    earlier build comes back without whatever has been added since. Every
    property here is reached by duck typing, so the first read of a missing one
    is a bare ``AttributeError`` from inside the adapter. The panel shows
    anything that is not a :class:`TranslationError` as an internal error with a
    traceback, which reads as a bug in the workbench rather than as a document
    out of step with it.

    Old documents stay unsupported. This function only makes being unsupported
    arrive as a sentence naming the object and the property.

    Three conditions have to hold before an error is rewritten, and together
    they keep the message from blaming the user for a fault in the workbench.
    The object has to be one of ours, by its proxy. It has to lack the
    attribute. And the attribute has to be spelled the way FreeCAD spells a
    property, in CapWords, which every property this adapter reads is and none
    of its own internals are.

    That still cannot separate a document missing a property from this adapter
    misspelling one, because the two are the same event seen from inside. The
    message
    therefore does not claim to know which, and says what to do in either case.
    """
